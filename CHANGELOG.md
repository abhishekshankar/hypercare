...
- Amplify deploy size: `amplify.yml` **postBuild** removes `apps/web/.next/cache` and the nested `standalone/.../.next/cache` (build caches duplicated into the standalone tree) so the artifact stays under Hosting’s ~220MB cap.
