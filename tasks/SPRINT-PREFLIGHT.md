...
3. **Client bundle / import boundary** — ESLint (value imports) plus **`pnpm --filter web run build` then `pnpm --filter web run check:client-bundle`** to grep compiled `.next/static` for `@hypercare/rag` / Bedrock (should find zero; complements lint).
