...
A one-shot migration (`packages/db/src/scripts/migrate-stage-v1.ts`):
...
- Data test (`pnpm --filter @hypercare/db migrate:stage-v1 --dry-run`): on the seeded beta fixture, report the migration summary; the "stage flipped" count is < 5%.
...
5. Run `pnpm --filter @hypercare/db migrate:stage-v1 --dry-run` against the seeded fixture → report shows < 5% stage flips.
